    ...
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-5">
      ...
      {items.length === 0 && !pagination ? (
        ...
        <div className="overflow-hidden rounded-xl border bg-card">
          <Table><TableHeader><TableRow><TableHead>Name</TableHead><TableHead className="hidden w-24 sm:table-cell">Type</TableHead><TableHead className="hidden w-28 md:table-cell">Size</TableHead><TableHead className="hidden w-44 lg:table-cell">Deleted</TableHead><TableHead className="w-56" /></TableRow></TableHeader>
          <TableBody>{items.map((item) => { const itemPending = pending.has(item.node.id); return <TableRow key={`${item.node.kind}:${item.node.id}`}>
            <TableCell><div className="flex min-w-0 items-center gap-2">{item.node.kind === "folder" ? <FolderIcon className="size-4 shrink-0" /> : <FileIcon className="size-4 shrink-0" />}<div className="min-w-0"><p className="truncate font-medium">{item.node.name}</p><p className="truncate text-xs capitalize text-muted-foreground sm:hidden">{item.node.kind}</p></div></div></TableCell>
