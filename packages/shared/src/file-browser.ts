...
export const DEFAULT_BROWSER_OPTIONS: BrowserOptions = { view: "list", sort: "name", order: "asc" }
...
export function browserSearchParams(options: BrowserOptions) {
...
export function browserURL(pathname: string, options: BrowserOptions) {
  const query = browserSearchParams(options).toString()
