...
export function TrashView({ initialPage }: { initialPage: TrashPage }) {
  const router = useRouter(); const workspace = useWorkspace(); const [items, setItems] = useState<TrashItem[]>(() => [...initialPage.items]); const [nextCursor, setNextCursor] = useState(initialPage.nextCursor); const [loading, setLoading] = useState(false); const [emptying, setEmptying] = useState(false); const [pending, setPending] = useState<ReadonlySet<string>>(() => new Set())
  ...
  async function loadMore() { if (!nextCursor || loading || emptying) return; setLoading(true); try { const query = { ownerId: workspace.id, limit: pageSize, cursor: nextCursor } satisfies TrashQuery; const page = await apiJSON<TrashPage>("/api/v1/trash", { query }); setItems((current) => [...current, ...page.items]); setNextCursor(page.nextCursor) } catch (error) { toast.error(apiErrorMessage(error, "Could not load more trash items.")); throw error } finally { setLoading(false) } }
  ...
  async function emptyTrash() { if (emptying || loading || pending.size > 0) return false; setEmptying(true); try { await apiJSON<void>("/api/v1/trash", { method: "DELETE", query: { ownerId: workspace.id } }); setItems([]); setNextCursor(undefined); toast.success("Trash emptied"); router.refresh(); return true } catch (error) { toast.error(apiErrorMessage(error, "Could not empty trash.")); return false } finally { setEmptying(false) } }
  return <TrashPresentation username={workspace.username} items={items} pending={pending} emptying={emptying} emptyDisabled={loading} onRestore={restore} onDeleteForever={deleteForever} onEmptyTrash={emptyTrash} renderDeletedAt={(item) => <DateOnly value={item.deletedAt} />} pagination={nextCursor ? <PaginationTrigger loadKey={nextCursor} hasMore loading={loading || emptying} onLoadMore={loadMore} className="p-3" loadingLabel="Loading more trash items…" /> : null} />
}
