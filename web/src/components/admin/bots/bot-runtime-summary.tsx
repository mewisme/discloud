...
export function BotRuntimeSummary({ snapshot }: { snapshot: BotRuntimeSnapshot }) {
  const stats = [
    ["Bots", snapshot.summary.configured],
      ...
      <div className="flex min-h-6 flex-wrap items-center gap-2 text-xs text-muted-foreground">
        ...
        {queues.length === 0 ? (
          ...
          <Badge key={operation} variant="outline" className="gap-1.5">
            ...
            {queue.oldestWaitMs > 0 && <span className="text-muted-foreground">· {formatWait(queue.oldestWaitMs)}</span>}
          </Badge>
        ))}
      </div>
    </section>
