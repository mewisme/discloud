...
import { CameraIcon, Loader2Icon, SaveIcon, Trash2Icon, UserRoundIcon } from "lucide-react"
...
import type { AvatarInfo, UpdateMeInput, User } from "@/lib/api/models"
...
export function ProfileSettings() {
  ...
  const avatarPending = uploading || removing
  ...
  async function saveName() {
    ...
    try {
      ...
      const updated = await apiJSON<User>("/me", { method: "PATCH", body: input })
      setUser(updated)
  ...
  async function selectAvatar(file: File) {
    ...
    try {
      ...
      setUser((current) => ({ ...current, hasAvatar: info.hasAvatar, avatarRevision: info.avatarRevision }))
  ...
  async function removeAvatar() {
    ...
    try {
      ...
      setUser((current) => ({ ...current, hasAvatar: false, avatarRevision: current.avatarRevision + 1 }))
      setRemoveOpen(false)
    ...
    <div className="space-y-6">
        ...
        <CardContent className="space-y-5">
          <Field>
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              <Input id="profile-name" autoComplete="name" maxLength={100} value={name} disabled={savingName} onChange={(event) => setName(event.target.value)} />
              <Button className="sm:w-auto" disabled={savingName || name.trim() === user.name} onClick={() => void saveName()}>
                {savingName ? <Loader2Icon className="animate-spin" /> : <SaveIcon />}
          ...
          <Field>
            ...
            <FieldDescription>Username is permanent. It is used to sign in and identify your workspace at /{user.username}.</FieldDescription>
          </Field>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Upload a profile picture for your DisCloud account. Images are normalized to a 512 × 512 avatar before storage.</CardDescription>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid gap-2 sm:flex">
            <Button className="w-full sm:w-auto" disabled={avatarPending} onClick={() => inputRef.current?.click()}>
            ...
            {user.hasAvatar && (
              <AlertDialog open={removeOpen} onOpenChange={changeRemoveOpen}>
                <AlertDialogTrigger asChild>
                  <Button className="w-full sm:w-auto" variant="outline" disabled={avatarPending}>
                    <Trash2Icon />
                    Remove avatar
                  </Button>
                </AlertDialogTrigger>
                ...
                <AlertDialogContent size="sm">
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>Your current avatar will be removed and DisCloud will fall back to your initials. You can upload another image at any time.</AlertDialogDescription>
                  </AlertDialogHeader>
