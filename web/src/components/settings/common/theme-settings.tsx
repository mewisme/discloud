    ...
    <Card id="theme" className="scroll-mt-24">
      ...
      <CardContent>
        ...
        >
          <Select
            value={effect}
            onValueChange={(value) => onEffectChange(value as ThemeEffect)}
          >
            <SelectTrigger
              className="w-full sm:w-56"
              aria-label="Theme transition effect"
            >
              <SelectValue />
            </SelectTrigger>
          ...
          >
            <div className="w-full space-y-3 sm:w-[36rem]">
              <Textarea
                ...
                placeholder={customThemeExample}
                spellCheck={false}
  ...
  return (
    <Collapsible className="group/docs">
      <CollapsibleTrigger asChild>
        ...
        >
          <ChevronRightIcon className="transition-transform duration-200 group-data-[state=open]/docs:rotate-90" />
      ...
      └─ ::view-transition-new(root)`}</code>
          ...
          <div className="space-y-2">
            ...
            <div className="grid gap-2 text-xs">
              <CSSReference
                selector="::view-transition-group(root)"
                description="Container for the root transition. Good place for shared duration and easing."
              />

              <CSSReference
                selector="::view-transition-image-pair(root)"
                description="Contains the old and new snapshots."
              />

              <CSSReference
                selector="::view-transition-old(root)"
                description="Snapshot of the theme before the change."
              />

              <CSSReference
                selector="::view-transition-new(root)"
                description="Snapshot of the new theme being revealed."
              />

              <CSSReference
                selector=".dark::view-transition-new(root)"
                description="Target a transition specifically when the resulting document is dark."
              />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid gap-1.5 text-xs text-muted-foreground">
              <span>
                Use <code className="text-foreground">clip-path</code> for circle, polygon and directional reveals.
              </span>

              <span>
                Use <code className="text-foreground">mask</code> for custom shapes and blurred edges.
              </span>

              <span>
                Use <code className="text-foreground">transform</code> and <code className="text-foreground">opacity</code> for slides, scales and fades.
              </span>

              <span>
                Add <code className="text-foreground">animation: none</code> to the old or new snapshot when you do not want the browser&apos;s default cross-fade.
              </span>

              <span>
                Define regular <code className="text-foreground">@keyframes</code> in the same custom CSS field.
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-2 border-t pt-3 text-xs">
            ...
            >
              MDN View Transition guide
              <ExternalLinkIcon className="size-3" />
            </a>

            <a
              href="https://developer.mozilla.org/en-US/docs/Web/CSS/Guides/View_transitions"
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1 text-muted-foreground underline-offset-4 hover:text-foreground hover:underline"
            >
