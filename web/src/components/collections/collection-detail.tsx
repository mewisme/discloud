    ...
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-5">
      ...
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
        <div className="min-w-0">
          ...
          <p className="mt-1 max-w-3xl text-sm text-muted-foreground">{collection.description || "No description"}</p>
        </div>

        {canEdit && (
          <div className="flex flex-wrap gap-2">
            <AddItemDialog collectionId={collection.id} existingItems={items} onAdded={reloadItems} />
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction variant="destructive" onClick={() => void trash()}>Trash collection</AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog>
          </div>
      ...
      {items.length === 0 ? (
        <div className="grid min-h-64 place-items-center rounded-xl border border-dashed p-6 text-center">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">{canEdit ? "Add files without moving them from their folders." : "Files will appear here when they are added."}</p>
          </div>
        </div>
      ) : (
                  ...
                  <TableCell>
                    <div className="flex justify-end">
                      <Button size="icon-sm" variant="ghost" asChild>
                        <a href={collectionDownloadURL(collection.id, item.fileId)} aria-label={`Download ${item.name}`}>
                          <DownloadIcon />
                        </a>
                      </Button>
                      {canEdit && (
                        <Button size="icon-sm" variant="ghost" aria-label={`Remove ${item.name}`} onClick={() => void removeItem(item.fileId)}>
                          <Trash2Icon />
                        </Button>
                      )}
                    </div>
...
function EditCollectionDialog({ collection, onUpdated }: { collection: Collection; onUpdated: (collection: Collection) => void }) {
      ...
      <DialogContent>
        ...
        <form className="space-y-4" onSubmit={form.handleSubmit(submit)}>
          ...
          <Field data-invalid={!!form.formState.errors.name}>
            ...
            <Input id="edit-collection-name" disabled={form.formState.isSubmitting} aria-invalid={!!form.formState.errors.name} {...form.register("name")} />
...
function AddItemDialog({ collectionId, existingItems, onAdded }: { collectionId: string; existingItems: readonly CollectionItem[]; onAdded: () => Promise<void> }) {
  ...
  async function add(fileId: string) {
    ...
    try {
      ...
      const result = await apiJSON<{ created: boolean }>(`/api/v1/collections/${collectionId}/items`, { method: "POST", body: input })
      await onAdded()
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>Choose files you can access through the folder hierarchy.</DialogDescription>
        </DialogHeader>
                ...
                return (
                  <div key={item.id} className="flex items-center gap-3 p-3">
                    ...
                    <Button size="sm" variant="outline" disabled={added || pendingId === item.id} onClick={() => void add(item.id)}>
                      {pendingId === item.id && <Loader2Icon className="animate-spin" />}
                      {added ? "Added" : "Add"}
                    </Button>
                  </div>
                )
