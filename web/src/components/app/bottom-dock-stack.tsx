...
export function BottomDockStackProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <BottomDockStackContext.Provider value={targets}>
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-[calc(1rem+env(safe-area-inset-bottom))] z-40 flex flex-col items-center gap-3 px-3">
        <div ref={uploadsRef} className="contents" />
        <div ref={selectionRef} className="contents" />
        <div ref={fileBrowserRef} className="contents" />
      </div>
