...
const uploadRetryDelaysMs = [300, 600, 1200] as const
  ...
  try {
    ...
    for (let attempt = 0; ; attempt++) {
      try {
        ...
        const retryDelayMs = uploadRetryDelaysMs[attempt]
        if (!(error instanceof APIError) || error.status !== 409 || retryDelayMs === undefined) return
