...
import { DesktopAddCollectionItemDialog, DesktopEditCollectionDialog, DesktopTrashCollectionButton } from "./collection-actions"
...
export function DesktopCollectionPage() {
  ...
  async function reloadItems() {
    ...
    const items = await apiJSON<CollectionItems>(`/api/v1/collections/${encodeURIComponent(state.collection.id)}/items`)
    setState((current) => current.status === "ready" ? { ...current, items: items.items } : current)
  }
  ...
  async function remove(item: CollectionItem) {
    ...
    try {
      await apiJSON<void>(
        `/api/v1/collections/${encodeURIComponent(state.collection.id)}/items/${encodeURIComponent(item.fileId)}`,
        { method: "DELETE" },
      )

      setState((current) => current.status === "ready"
        ? { ...current, items: current.items.filter((candidate) => candidate.fileId !== item.fileId) }
        : current)
    } catch (error) {
  ...
  if (state.status === "error" || !username) {
    return (
      <p role="alert" className="text-sm text-destructive">
        {state.status === "error" ? state.message : "Workspace username is missing."}
      </p>
    )
  }
  ...
  const full = state.collection.accessLevel === "full"
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-5">
      <FileBreadcrumbs items={[
        ...
        {
          id: `collection:${state.collection.id}`,
          label: state.collection.name,
        },
      ]} />
      ...
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
        <div className="min-w-0">
          ...
          <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
            {state.collection.description || "No description"}
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {canEdit ? (
            <>
              <DesktopAddCollectionItemDialog
                collectionId={state.collection.id}
                existingItems={state.items}
                onAdded={reloadItems}
              />

              <DesktopEditCollectionDialog
              ...
              <DesktopTrashCollectionButton
                collection={state.collection}
                onTrashed={() => navigate(collectionsPath, { replace: true })}
              />
            </>
          ) : null}
          ...
          {full ? (
            <>
              ...
              <DesktopPublicShareDialog
                ...
                resourceId={state.collection.id}
                resourceName={state.collection.name}
              />
            </>
          ) : null}
        </div>
                ...
                <TableRow key={item.fileId}>
                  <TableCell>
                    <div className="flex min-w-0 items-center gap-2">
                      ...
                      <Link
                        to={workspaceCollectionFilePath(username, state.collection.id, item.fileId)}
                  ...
                  <TableCell>
                    <div className="flex justify-end">
                      <Button
                        size="icon-sm"
                        variant="ghost"
                        disabled={pending.has(item.fileId)}
                        aria-label={`Download ${item.name}`}
                        onClick={() => void download(item)}
                      >
                        {pending.has(item.fileId)
