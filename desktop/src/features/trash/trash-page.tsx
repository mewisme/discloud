...
export function DesktopTrashPage() {
  ...
  return <TrashView username={state.workspace.owner.username} items={state.page.items} pending={pending} actionError={actionError} onRestore={restore} onDeleteForever={deleteForever} pagination={state.page.nextCursor ? <div className="flex justify-center p-3"><Button variant="outline" disabled={loadingMore} onClick={() => void loadMore()}>{loadingMore ? <Loader2Icon className="animate-spin" /> : null}{loadingMore ? "Loading" : "Load more"}</Button></div> : null} />
}
