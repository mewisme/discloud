...
export function DesktopTrashPage() {
  const { username } = useParams(); const [state, setState] = useState<State>({ status: "loading" }); const [loadingMore, setLoadingMore] = useState(false); const [emptying, setEmptying] = useState(false); const [pending, setPending] = useState<ReadonlySet<string>>(() => new Set()); const [actionError, setActionError] = useState<string>()
  ...
  async function loadMore() { if (state.status !== "ready" || !state.page.nextCursor || loadingMore || emptying) return; setLoadingMore(true); setActionError(undefined); try { const page = await apiJSON<TrashPage>("/api/v1/trash", { query: trashQuery(state.workspace.owner.id, state.page.nextCursor) }); setState((current) => current.status === "ready" ? { ...current, page: { ...page, items: [...current.page.items, ...page.items] } } : current) } catch (error) { setActionError(errorMessage(error)); throw error } finally { setLoadingMore(false) } }
  ...
  async function emptyTrash() { if (state.status !== "ready" || emptying || loadingMore || pending.size > 0) return false; setEmptying(true); setActionError(undefined); try { await apiJSON<void>("/api/v1/trash", { method: "DELETE", query: { ownerId: state.workspace.owner.id } }); setState((current) => current.status === "ready" ? { ...current, page: { items: [] } } : current); return true } catch (error) { setActionError(errorMessage(error)); return false } finally { setEmptying(false) } }
  ...
  return <TrashView username={state.workspace.owner.username} items={state.page.items} pending={pending} emptying={emptying} emptyDisabled={loadingMore} actionError={actionError} onRestore={restore} onDeleteForever={deleteForever} onEmptyTrash={emptyTrash} pagination={state.page.nextCursor ? <DesktopPaginationTrigger loadKey={state.page.nextCursor} hasMore loading={loadingMore || emptying} onLoadMore={loadMore} className="p-3" loadingLabel="Loading more trash items…" /> : null} />
}
