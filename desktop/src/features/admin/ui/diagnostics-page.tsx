  ...
  return (
    <div className="space-y-3">
      ...
      <div className="overflow-hidden rounded-xl border">
        <Table>
          <TableHeader><TableRow><TableHead>Time</TableHead><TableHead>Action</TableHead><TableHead className="hidden lg:table-cell">Actor</TableHead><TableHead className="hidden md:table-cell">Resource</TableHead><TableHead className="w-12" /></TableRow></TableHeader>
          <TableBody>
            {events.map((event) => <TableRow key={event.id}><TableCell className="whitespace-nowrap text-muted-foreground">{formatDateTime(event.createdAt)}</TableCell><TableCell><Badge variant="outline" className="font-mono font-normal">{event.action}</Badge></TableCell><TableCell className="hidden lg:table-cell">{event.actorName || (event.actorUsername ? `@${event.actorUsername}` : event.actorUserId || "system")}</TableCell><TableCell className="hidden md:table-cell"><div className="max-w-64"><p className="truncate text-sm">{event.resourceType || "—"}{event.resourceName ? ` · ${event.resourceName}` : event.resourceUsername ? ` · @${event.resourceUsername}` : ""}</p>{event.resourceId ? <p className="truncate font-mono text-[11px] text-muted-foreground">{event.resourceId}</p> : null}</div></TableCell><TableCell><JSONDialogButton title={event.action} value={event.metadata} /></TableCell></TableRow>)}
...
function UploadDiagnostics({ initialPage }: { initialPage: UploadDiagnosticPage }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {error ? <InlineError message={error} /> : null}
      <div className="overflow-hidden rounded-xl border">
        <Table><TableHeader><TableRow><TableHead>Updated</TableHead><TableHead>Name</TableHead><TableHead>Status</TableHead><TableHead className="hidden md:table-cell">Parts</TableHead><TableHead className="hidden lg:table-cell">Size</TableHead><TableHead className="hidden xl:table-cell">Failures</TableHead><TableHead className="w-12" /></TableRow></TableHeader><TableBody>{uploads.map((upload) => <TableRow key={upload.id}><TableCell className="whitespace-nowrap text-muted-foreground">{formatDateTime(upload.updatedAt)}</TableCell><TableCell><div className="max-w-72"><p className="truncate font-medium">{upload.name}</p><p className="truncate text-xs text-muted-foreground">Owner: {upload.ownerName} · @{upload.ownerUsername}</p><p className="truncate text-xs text-muted-foreground">Actor: {upload.actorName} · @{upload.actorUsername}</p></div></TableCell><TableCell><StatusBadge status={upload.status} /></TableCell><TableCell className="hidden tabular-nums md:table-cell">{formatNumber(upload.uploadedParts)} / {formatNumber(upload.expectedParts)}</TableCell><TableCell className="hidden tabular-nums lg:table-cell">{formatBytes(upload.sizeBytes)}</TableCell><TableCell className="hidden tabular-nums xl:table-cell">{formatNumber(upload.failedAttempts)} / {formatNumber(upload.attemptCount)}</TableCell><TableCell><JSONDialogButton title={upload.name} value={upload} /></TableCell></TableRow>)}{!loading && !uploads.length ? <TableRow><TableCell colSpan={7} className="h-24 text-center text-muted-foreground">No uploads found.</TableCell></TableRow> : null}</TableBody></Table>
        <LoadMore loading={loading} nextCursor={nextCursor} onLoad={() => nextCursor ? run({ ...query, cursor: nextCursor }, true) : Promise.resolve()} />
      </div>
    </div>
  )
