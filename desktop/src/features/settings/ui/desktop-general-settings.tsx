...
export function DesktopGeneralSettings() {
  ...
  if (runtime.loading && !runtime.preferences) {
    return <div className="grid min-h-64 place-items-center"><div className="flex items-center gap-2 text-sm text-muted-foreground"><Loader2Icon className="animate-spin" />Loading desktop settings</div></div>
  }
  ...
  return (
    <section className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Configure native startup, tray and notification behavior for this device.</p>
      </div>
        ...
        <CardContent>
          ...
          <SettingRow title="Operating-system permission" description="DisCloud can only display notifications after the operating system grants permission." last>
            <div className="flex flex-wrap items-center justify-end gap-2">
              ...
              <Button size="sm" variant="outline" disabled={!!pending} onClick={() => void testNotification()}>{pending === "test-notification" ? <Loader2Icon className="animate-spin" /> : <BellIcon />}Test notification</Button>
...
function SettingRow({ title, description, children, last = false }: { title: string; description: string; children: ReactNode; last?: boolean }) {
  return <div className={`grid gap-4 py-5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center ${last ? "" : "border-b"} first:pt-0 last:pb-0`}><div><p className="text-sm font-medium">{title}</p><p className="mt-1 text-sm text-muted-foreground">{description}</p></div><div className="sm:flex sm:justify-end">{children}</div></div>
}

function RuntimeStatus({ title, description }: { title: string; description: string }) {
  return <div className="rounded-xl border p-4"><div className="flex items-center justify-between gap-3"><p className="font-medium">{title}</p><Badge>Enabled</Badge></div><p className="mt-2 text-sm text-muted-foreground">{description}</p></div>
}
