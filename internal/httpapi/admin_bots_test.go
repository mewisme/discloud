...
func (p *fakeBotRuntimeProvider) RuntimeSnapshot() discordstore.BotRuntimeSnapshot {
	return p.snapshot
}
...
func TestBotRuntimeSnapshotHandler(t *testing.T) {
	...
	if response.Summary.Configured != 2 ||
		response.Summary.Resolved != 2 ||
		response.Summary.Unresolved != 0 ||
	...
	bot := response.Bots[0]
	if bot.ID == nil || *bot.ID != "123" || bot.Username != "storage-bot" || bot.DisplayName != "Storage Bot" {
		t.Fatalf("bot identity = %+v", bot)
	}
	if bot.ConfigIndex != 0 || !bot.Resolved {
		t.Fatalf("bot config identity = %+v", bot)
	}
	if bot.AvatarURL != "https://cdn.discordapp.com/avatars/123/avatar-hash.png?size=128" {
		t.Fatalf("avatarUrl = %q", bot.AvatarURL)
	}
	if bot.Lease == nil || bot.Lease.Operation != "upload" || bot.Lease.PartIndex == nil || *bot.Lease.PartIndex != 7 {
...
func TestBotRuntimeEventsHandlerReplaysEvents(t *testing.T) {
	...
	cancel()
	request := httptest.NewRequest(http.MethodGet, "/api/v1/admin/bots/events", nil).WithContext(ctx)
	...
	for _, want := range []string{"event: ready", "id: 2", "event: bot.lease.finished", `"botId":"123"`} {
...
func TestBotRuntimeEventsHandlerRequestsResetOnReplayGap(t *testing.T) {
	provider := &fakeBotRuntimeProvider{
		window: discordstore.RuntimeEventWindow{
			OldestID: 5,
			LatestID: 8,
			Events:   []discordstore.RuntimeEvent{{ID: 5, Type: discordstore.RuntimeEventQueueChanged}},
		},
		events: make(chan discordstore.RuntimeEvent),
	}
...
func TestBotRuntimeEventsHandlerRejectsInvalidLastEventID(t *testing.T) {
	provider := &fakeBotRuntimeProvider{}
	request := httptest.NewRequest(http.MethodGet, "/api/v1/admin/bots/events", nil)
...
func TestRuntimeReplayGap(t *testing.T) {
	tests := []struct {
		...
		{name: "new client", after: 0, window: discordstore.RuntimeEventWindow{OldestID: 5, LatestID: 10}},
		{name: "replay available", after: 4, window: discordstore.RuntimeEventWindow{OldestID: 5, LatestID: 10}},
		{name: "ring buffer gap", after: 2, window: discordstore.RuntimeEventWindow{OldestID: 5, LatestID: 10}, want: true},
		{name: "server restarted", after: 100, window: discordstore.RuntimeEventWindow{LatestID: 3}, want: true},
	}
